...
googledrive::drive_download(file=fn,path="data/SGR_Review",overwrite=TRUE)

### Search data
SearchHits <- readxl::read_excel("data/SGR_Review.xlsx",sheet="Hits") %>%
...
yrs <- 2009:2011
#### Create a data.frame to receive the results
tmp <- readxl::read_excel("data/SGR_Review.xlsx",sheet="2009") %>%
...
for (i in yrs) {
  ...
  res <- readxl::read_excel("data/SGR_Review.xlsx",sheet=as.character(i)) %>%
